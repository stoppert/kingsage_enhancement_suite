css +=
    "#kes_box { background-color: #fff; display: none; border-radius: 5px; position: fixed; width: 500px; left: 50%; top: 5%; margin-left: -250px; padding: 6px; z-index: 200; }";
css +=
    ".kes_progressOuter { border:1px solid #CFAB65; overflow: hidden; width:100px; height:10px; background: #CFAB65; }";
css += ".kes_progressInner { width: 0px; height: 10px; background: #FFCC6E; }";

(function(kes) {
    kes.module.infoally = {
        matcher:
            (page.match("s=info_ally") && !premium) ||
            identifyActiveTab("s=info_ally&m=profile"),
        fn: function() {
            /* FUNCTIONALITY
            ...
            cachedTable.append(
                '<tr><td colspan="2"><span class="click" id="kes_getAllyTroopPoints">' +
                    l.allyTrooppoints +
                    "</span></td></tr>"
            );
                ...
                $.when(
                    $.ajax({
                        ...
                        function getAllPlayerInfos(members, callback) {
                            ...
                            $("#kes_getAllyTroopPoints")
                                .removeClass("click")
                                .text(l.loading + ": " + i + "/" + len)
                                .fadeIn()
                                .parent()
                                .attr("colspan", 0)
                                .after(
                                    '<td><div class="kes_progressOuter"><div id="kes_progress" class="kes_progressInner" maxwidth="100"></div></div></td>'
                                );

                            function callQueue(members) {
                                getPlayerInfo(members, function(
                                    members,
                                    playerInfo
                                ) {
                                    var player = [];
                                    var playerName = $(playerInfo)
                                        .find("h1")
                                        .eq(1)
                                        .text();
                                    ...
                                    player.push(
                                        calculateTroopScore(playerInfo)
                                    );

                                    i++;
                                    $("#kes_getAllyTroopPoints").text(
                                        l.loading + ": " + i + "/" + len
                                    );
                                    $("#kes_progress").css({
                                        width: i * (100 / len) + "%",
                        ...
                        function showData(allyTroopPoints) {
                            ...
                            var o =
                                arrow +
                                ' <span class="click" id="kes_createBB">(kes) ' +
                                l.bbCode +
                                '</span><table class="borderlist" style="width: 420px;">';
                            o +=
                                "<tr><th>" +
                                l.player +
                                "</th><th>" +
                            ...
                            $("#kes_createBB").bind("click", function() {
                                $("body").append(
                                    '<div id="kes_box"><textarea style="width: 99%; resize: none;" id="kes_data"></textarea></div><div id="kes_overlay" class="kes-backlight"></div>'
                                );
                                $("#kes_data")
                                    .text(
                                        getDataReady(
